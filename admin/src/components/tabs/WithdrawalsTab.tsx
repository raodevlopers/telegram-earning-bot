...
export function WithdrawalsTab({ withdrawals, onReview }: WithdrawalsTabProps) {
  return (
    <Panel title="Withdrawal queue" subtitle="Approvals keep the hold in place. Rejections refund the user automatically through the wallet ledger.">
        ...
        <table>
          ...
          <tbody>
            {withdrawals.length ? (
              ...
              <tr>
                <td colSpan={6}>
                  <p className="empty-copy">No withdrawal requests have been raised yet.</p>
                </td>
              </tr>
            )}
          </tbody>
